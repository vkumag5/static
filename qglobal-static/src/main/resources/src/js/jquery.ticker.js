	...
	$.fn.ticker = function(options) { 
		...
		return this.each(function() { 
					...
					if (opts.ajaxFeed) {
						...
						if($(newsID + ' LI').length > 0) {
							$(newsID + ' LI').each(function (i) {	
								...
								if($(this).attr("title") == 'alert') {  //to set the style of the alert/news
									settings.newsArr['item-' + i] = { type: opts.titleText, content: $(this).html(), listClass : "alert"};
								}
								else {
									settings.newsArr['item-' + i] = { type: opts.titleText, content: $(this).html(), listClass : "news"};
								}
			...
			function setupContentAndTriggerDisplay() {
				...
				$(settings.dom.contentID).removeClass("alert news");
				$(settings.dom.contentID).addClass(settings.newsArr['item-' + settings.position].listClass);
