...
(function ($) {
	...
	function Wysiwyg() {
		...
		this.controls = {
			...
			insertToken: {
				...
				exec: function () {
					...
					if ($.wysiwyg.controls && $.wysiwyg.controls.token) {
						$.wysiwyg.controls.token(this);
					} else if ($.wysiwyg.autoload) {
						$.wysiwyg.autoload.control("wysiwyg.insertToken.js", function () {
							self.controls.insertToken.exec.apply(self);
						});
					} else {
						console.error("$.wysiwyg.controls.token not defined. You need to include wysiwyg.insertToken.js file");
					}
				}
		...
		this.defaults = {
html: '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" style="margin:0"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"></head><body style="margin:0;">INITIAL_CONTENT</body></html>',
			debug: false,
			controls: {},
			css: {},
			events: {},
		...
		this.getInternalSelection = function () {
			...
			try{
			var iframeObj = $("editEmailTemplateForm:wysiwyg-wysiwyg-iframe");
			if (iframeObj.contentWindow) {				
				if (iframeObj.contentWindow.getSelection) {
					return iframeObj.contentWindow.getSelection();
				}
				if (iframeObj.contentWindow.document.selection) {
					return iframeObj.contentWindow.document.selection;
				}
	...
	$(function () { // need access to jQuery UI stuff.
		...
		$.wysiwyg.dialog.register("default", function () {
			...
			this.destroy = function() {
				...
				// Modal feature:
				if (this.options.modal) { 
					that._$dialog.parent().each(function(){
